...
| **4** | Удаление файлов спецификаций — удаляет в выбранных папках все файлы вида `* fcs.xlsx` и `* fcs.xls`. |
| **0** | Выход. |

## Диапазон номеров инвойсов

После выбора сценария 1, 2 или 4 программа запрашивает номера инвойсов. Можно вводить:

- отдельные номера: `1`, `5`, `10`
- диапазоны: `1-5`, `10-12`
- комбинации через запятую или точку с запятой: `1, 3, 5-7`, `1; 2; 3-5`

Скрипт ищет **папки**, в имени которых в начале встречается указанный номер (например, папка `123 Инвойс` подойдёт для номера `123`). В каждой такой папке ожидается файл `Invoice N.xlsx` (где N — номер).
